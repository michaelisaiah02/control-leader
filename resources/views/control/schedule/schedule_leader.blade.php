...
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-between mb-3">
            <div class="col-auto">
                <form action="{{ route('control.schedule.index') }}" method="get">
    ...
    @if (!empty($targets))
        <script type="module">
            ...
            async function handleShiftChange(sel) {
                ...
                try {
                    const res = await post("{{ route('control.schedule.updateCell', $plan->id) }}", {
                        user_id: userId,
                        date,
                        shift,
                        division,
            ...
            async function handleDivisionChange(sel) {
                ...
                try {
                    await post("{{ route('control.schedule.updateDivision', $plan->id) }}", {
                        user_id: userId,
                        division: division,
                    });
                } catch {
