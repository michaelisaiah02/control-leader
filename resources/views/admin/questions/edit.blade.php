...
@section('content')
    <form method="POST" action="{{ route('admin.question.update', $question->id) }}" class="px-5">
        @csrf
        @method('PUT')
        <!-- Header -->
        <div class="d-flex gap-5 w-100 mt-2 justify-content-between align-items-center my-2">
            <div class="d-flex align-items-center gap-2 w-100">
                ...
                <input id="question_text" type="text" name="question_text" class="form-control bg-warning-subtle"
                    value="{{ $question->question_text }}" required>
            </div>
            <div class="d-flex align-items-center gap-2 w-100">
                <label for="package"
                    class="form-label bg-primary text-white px-4 py-2 rounded shadow border border-white">Type</label>
                <select name="package" id="package" class="form-select bg-warning-subtle" required>
                    <option value="" selected disabled>-- Pilih</option>
                    <option value="awal_shift"
                        {{ old('package', $question->package ?? '') == 'awal_shift' ? 'selected' : '' }}>
                        Awal Shift
                    </option>
                    <option value="saat_bekerja"
                        {{ old('package', $question->package ?? '') == 'saat_bekerja' ? 'selected' : '' }}>
                        Saat Bekerja
                    </option>
                    <option value="setelah_istirahat"
                        {{ old('package', $question->package ?? '') == 'setelah_istirahat' ? 'selected' : '' }}>
                        Setelah Istirahat
                    </option>
                    <option value="akhir_shift"
                        {{ old('package', $question->package ?? '') == 'akhir_shift' ? 'selected' : '' }}>
                        Akhir Shift
                    </option>
                    <option value="leader" {{ old('package', $question->package ?? '') == 'leader' ? 'selected' : '' }}>
                        Leader
                    </option>
                </select>
            </div>
        ...
        <div class="row my-2">
                    ...
                    @if ($question->extra_fields)
                        ...
                        <div class="mb-3 border border-1 border-primary p-3 rounded field-block">
                            ...
                            <input type="text" name="problem_label"
                                class="form-control border border-1 border-primary bg-warning-subtle"
                                value="{{ $question->problem_label }}" required>
                        </div>
                        <div class="mb-3 border border-1 border-primary p-3 rounded field-block">
                            <label for="countermeasure_label" class="form-label my-2">Label Countermeasure</label>
                            <input type="text" id="countermeasure_label" name="countermeasure_label"
                                class="form-control border border-1 border-primary bg-warning-subtle"
                                value="{{ $question->countermeasure_label }}" required>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="d-grid gap-2 border border-primary p-3 rounded h-100">
                    <button type="button" class="btn btn-outline-primary add-field" data-type="radio">Tambah
                        Pilihan</button>
                    <button type="button"
                        class="btn btn-outline-primary add-field {{ $question->extra_fields === true ? 'active' : '' }}"
                        data-type="toggle">Problem dan
        ...
        <div class="py-1 d-flex justify-content-between">
            ...
            <div>
                <a href="{{ route('admin.question.index') }}" class="btn btn-primary text-white py-2 px-4">Back</a>
...
@section('scripts')
    ...
    `;
        ...
        <div class="mb-3 border border-1 border-primary p-3 rounded field-block">
          <label for="problem_label" class="form-label my-2">Label Problem</label>
          <input type="text" id="problem_label" name="problem_label" class="form-control border border-1 border-primary bg-warning-subtle" value="{{ $question->problem_label ? $question->problem_label : 'Problem' }}" required>
        </div>
        <div class="mb-3 border border-1 border-primary p-3 rounded field-block">
            <label for="countermeasure_label" class="form-label my-2">Label Countermeasure</label>
            <input type="text" id="countermeasure_label" name="countermeasure_label" class="form-control border border-1 border-primary bg-warning-subtle" value="{{ $question->countermeasure_label ? $question->countermeasure_label : 'Countermeasure' }}" required>
        </div>
      `);
                ...
                fields.forEach(input => {
                    input.closest(".field-block").remove()
                });
