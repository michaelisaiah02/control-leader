    ...
    <form method="GET">
        ...
        <div class="d-flex align-items-center gap-2">
            <label for="department"
                class="col-md-2 text-center form-label bg-primary text-white px-4 py-2 rounded shadow border border-white">Department</label>
        ...
        <div class="d-flex align-items-center gap-2">
            <label for="supervisor_name"
                class="col-md-2 text-center form-label bg-primary text-white px-4 py-2 rounded shadow border border-white">Nama
                Supervisor</label>
            <select name="supervisor_name" id="supervisor_name" class="form-control bg-warning-subtle" required>
                ...
                <option value="{{ $supervisor->id }}"> {{ $supervisor->employeeID }} - {{ $supervisor->name }} </option>
                @endforeach
            </select>
        </div>
        <div class="d-flex align-items-center gap-2">
            ...
            <select name="leader_name" id="leader_name" class="form-control bg-warning-subtle" required>
                ...
                <option value="{{ $leader->id }}">{{ $leader->employeeID }} - {{ $leader->name }} </option>
                @endforeach
            </select>
        </div>
        @break
        ...
        <div class="d-flex align-items-center gap-2">
            <p class="col-md-2 text-center form-label bg-primary text-white px-4 py-2 rounded shadow border border-white">
                Periode</p>
            <div class="d-flex justify-content-center align-items-center gap-2 w-100">
                <div class="d-flex align-items-center gap-2 w-100">
                    <label for="month"
                        class="col-md-2 text-center form-label bg-primary text-white px-4 py-2 rounded shadow border border-white">Month
                        & Year</label>
        ...
        <div class="my-3 pb-5 d-flex justify-content-center align-items-center gap-5">
            <button formaction="{{ route('reports.monthly', ['type' => $type]) }}"
                class="btn btn-primary btn-lg h-100 py-3 fs-4 align-content-center rounded-4 position-relative text-uppercase">View
                Leader Consistency Report</button>
            <button formaction="{{ route('reports.score', ['type' => $type]) }}"
                class="btn btn-primary btn-lg h-100 py-3 fs-4 align-content-center rounded-4 position-relative text-uppercase">View
                Score Leader Report</button>
        </div>
        @break

        @case('supervisor')
        <div class="my-3 pb-5 d-flex justify-content-center align-items-center gap-5">
            <a href="{{ route('reports.monthly', ['type' => $type]) }}"
                class="btn btn-primary btn-lg w-100 h-100 py-3 fs-4 align-content-center rounded-4 position-relative text-uppercase">View
                Supervisor Consistency Report</a>
            <a href="{{ route('reports.score', ['type' => $type]) }}"
                class="btn btn-primary btn-lg w-100 h-100 py-3 fs-4 align-content-center rounded-4 position-relative text-uppercase">View
                Score Supervisor Report</a>
        </div>
        @break
        ...
        <div>
            <a href="{{ route('reports.index') }}" class="btn btn-primary text-white py-2 px-4">Back</a>
        </div>
    </form>
</div>
@endsection