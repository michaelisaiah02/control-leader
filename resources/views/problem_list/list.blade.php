...
@section('content')
    <div class="px-5">
        ...
        @switch($type)
            @case('leader-performance')
                @include('problem_list.table._leader-performance', ['Problems' => $Problems])
            @break

            @case('leader-consistency')
                @include('problem_list.table._leader-consistency', ['Problems' => $Problems])
            @break

            @case('supervisor-performance')
                @include('problem_list.table._supervisor-performance', ['Problems' => $Problems])
            @break

            @case('supervisor-consistency')
                @include('problem_list.table._supervisor-consistency', ['Problems' => $Problems])
            @break
        ...
        <div class="py-1 d-flex justify-content-between">
            <div>
                <a href="{{ route('listProblem.index') }}" class="btn btn-primary text-white py-2 px-4">Back</a>
            </div>
        </div>
    </div>
@endsection
